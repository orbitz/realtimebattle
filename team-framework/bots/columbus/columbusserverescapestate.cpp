/************************************************************************
    $Id: columbusserverescapestate.cpp,v 1.6 2005/10/07 07:33:51 krolfy Exp $
    ...
    $Log: columbusserverescapestate.cpp,v $
...
void ColumbusServerEscapeState::sendNavigationServerTips() throw(bad_exception)
{
_sssp->getLogger()->logMessage(1,"## ESC ##");
		Position pos=_sssp->getMap().navigationHint(_issp->getRobotInfo().pos,_sssp->currentEnemyPosition(),-1,.3,1.5,0);
...
void ColumbusServerEscapeState::handleEnemy(const General::Position& pos) throw(bad_exception)
{
	_sssp->getEnemies().addEnemyPosition(_myEnemy=pos);
}

void ColumbusServerEscapeState::makeFootPrint(const General::Position& pos,const General::Position& speed) throw(bad_exception) {}

ColumbusServerEscapeState::ColumbusServerEscapeState(ServerCoordinator* c,ColumbusInternalServerSpecificRepository* columbusssp, ColumbusSharedServerSpecificRepository* columbussharedssp) throw (StrategyException,bad_exception): ColumbusServerBaseState(c,columbusssp,columbussharedssp),_sssp(columbussharedssp),_issp(columbusssp) {}

ColumbusServerEscapeState::~ColumbusServerEscapeState () throw() {}
